...
const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose, initialData }) => {
  ...
  const getChildPolicy = (age: number) => {
    if (age <= 3) return { label: 'Berço Cortesia', cost: 0, icon: <Baby className="w-3 h-3" /> };
    if (age <= 8) return { label: 'Cama Júnior', cost: 20, icon: <Bed className="w-3 h-3" /> };
    return { label: 'Cama Juvenil', cost: 40, icon: <Bed className="w-3 h-3" /> };
  };
  ...
  const handleNext = () => {
    setError(null);
    if (step === 1) {
      if (!checkIn) { triggerError("Por favor, selecione a data de check-in."); return; }
      if (!checkOut) { triggerError("Por favor, selecione a data de check-out."); return; }
      const [y1, m1, d1] = checkIn.split('-').map(Number);
      const [y2, m2, d2] = checkOut.split('-').map(Number);
      const start = new Date(y1, m1 - 1, d1);
      const end = new Date(y2, m2 - 1, d2);
      if (start >= end) { triggerError("A data de check-in não pode ser posterior ou igual à data de check-out."); return; }
      if (checkIn < todayStr) { triggerError("A data de check-in não pode ser no passado."); return; }
      setStep(2);
    } else if (step === 2) {
      if (selectedRoomIds.length === 0) { triggerError("Selecione pelo menos uma suite para continuar."); return; }
      if (!isCapacitySufficient) {
        let msg = "";
        if (remainingAdults > 0 && remainingChildren > 0) {
          msg = `Capacidade excedida: As suites selecionadas não comportam ${remainingAdults} adultos e ${remainingChildren} crianças adicionais.`;
        } else if (remainingAdults > 0) {
          msg = `Capacidade excedida: O número de adultos excede a capacidade das suites selecionadas (faltam ${remainingAdults}).`;
        } else {
          msg = `Capacidade excedida: Não existem camas extra suficientes para ${remainingChildren} crianças nas suites selecionadas.`;
        }
        triggerError(msg);
        return;
      }
      ...
      if (!billingDetails.firstName || !billingDetails.lastName || !billingDetails.email || !billingDetails.phone) {
        triggerError("Por favor, preencha todos os dados de contacto obrigatórios.");
        return;
      }
      ...
      <div className="relative w-full max-w-5xl bg-bone h-full md:h-auto md:max-h-[95vh] overflow-hidden flex flex-col rounded-sm shadow-2xl animate-fade-up">
        ...
        <div className="bg-bone border-b border-charcoal/5 px-4 py-4 md:px-8 md:py-6 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <h2 className="font-serif text-lg md:text-2xl text-charcoal tracking-tight">Reserva de Luxo</h2>
          </div>
          <button 
            type="button"
            onClick={onClose} 
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {step === 1 && (
              <div className="space-y-8 md:space-y-16 animate-fade-up">
                <div className="text-center space-y-2 md:space-y-4">
                  <h3 className="font-serif text-3xl md:text-6xl text-charcoal">Datas & Hóspedes</h3>
                  <p className="text-charcoal/40 text-xs md:text-sm italic font-light">Defina o seu grupo para encontrarmos o melhor espaço.</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-20">
                  <div className="relative group border border-charcoal/5 hover:border-gold transition-colors p-4 md:p-6 rounded-sm bg-white">
                    <div className="pointer-events-none">
                      <span className="block text-[8px] md:text-[10px] uppercase tracking-[0.4em] font-black text-charcoal/30 mb-2 md:mb-4 group-hover:text-gold transition-colors">Entrada</span>
                  ...
                  <div className="relative group border border-charcoal/5 hover:border-gold transition-colors p-4 md:p-6 rounded-sm bg-white">
                    <div className="pointer-events-none">
                      <span className="block text-[8px] md:text-[10px] uppercase tracking-[0.4em] font-black text-charcoal/30 mb-2 md:mb-4 group-hover:text-gold transition-colors">Saída</span>
                ...
                <div className="space-y-4 md:space-y-6">
                  <div className="flex items-center justify-between p-4 md:p-8 bg-white shadow-sm border border-charcoal/5 rounded-sm transition-all hover:shadow-md hover:border-gold/20">
                    <div className="space-y-1">
                      <span className="text-[9px] md:text-[10px] uppercase tracking-[0.4em] font-black text-charcoal/30 block">Adultos</span>
                      <p className="hidden md:block text-[10px] text-gold font-bold uppercase tracking-widest">Acomodação Principal</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:gap-4">
                    ...
                    {childrenAges.length === 0 && (<div className="p-6 md:p-8 border border-dashed border-charcoal/10 rounded-sm text-center"><p className="text-[9px] md:text-[10px] uppercase tracking-widest text-charcoal/30">Sem crianças adicionadas</p></div>)}
                  </div>
                </div>

                <button type="button" onClick={handleNext} className="w-full bg-charcoal text-white py-6 md:py-8 uppercase text-[10px] md:text-[11px] tracking-[0.5em] font-black hover:bg-gold hover:shadow-2xl hover:shadow-gold/20 transition-all duration-500 rounded-sm">Procurar Disponibilidade</button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-8 md:space-y-12 animate-fade-up">
                <div className="text-center space-y-2 md:space-y-4">
                  <h3 className="font-serif text-3xl md:text-6xl text-charcoal">Seleção de Suites</h3>
                  <p className="text-charcoal/40 text-xs md:text-sm italic font-light">Selecione as suites para o seu grupo.</p>
                </div>
                {isPrestigeGuest && (
                  <div className="bg-gold/10 border border-gold/30 p-3 md:p-4 rounded-sm flex items-center gap-4 animate-fade-up">
                    ...
                    <span className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] font-black text-gold">Hóspede de Prestígio: Conforto Superior Detetado</span>
                  </div>
                )}
                <div className={`p-4 md:p-8 border rounded-sm flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 transition-all duration-700 ${isCapacitySufficient ? 'bg-white border-gold/20 shadow-md' : 'bg-gold/5 border-gold/10'}`}>
                  <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto">
                    ...
                    <div><p className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] font-black text-charcoal/40">Ocupação da Comitiva</p><p className="text-xs md:text-sm text-charcoal font-medium mt-1">{isCapacitySufficient ? "Espaço ideal confirmado." : `Acomode mais ${remainingAdults} adultos ${remainingChildren > 0 ? `e ${remainingChildren} crianças` : ''}`}</p></div>
                  </div>
                  {!isCapacitySufficient && (<button type="button" onClick={suggestOptimalRooms} className="w-full md:w-auto flex justify-center items-center gap-3 bg-gold text-white px-6 py-3 rounded-sm text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] hover:bg-charcoal hover:shadow-xl transition-all"><Sparkles className="w-3 h-3 md:w-4 md:h-4" /> Sugestão de Curadoria</button>)}
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-grow flex flex-col justify-between p-5 md:p-0 md:py-1">
                          ...
                          {isSelected && (<div className="flex items-center gap-3 mt-5 pt-4 border-t border-gold/5 md:border-0 md:mt-4 md:pt-0 animate-fade-up"><div className="w-5 h-5 bg-gold rounded-full flex items-center justify-center shadow-lg shadow-gold/20"><Check className="w-3 h-3 text-white" strokeWidth={4} /></div><span className="text-[10px] md:text-[11px] uppercase tracking-[0.3em] font-black text-gold">Suite Confirmada</span></div>)}
                        </div>
                ...
                <div ref={scrollAnchorRef} className="pt-4">
                  <button 
                    ...
                    onClick={handleNext} 
                    className={`
                      w-full py-6 md:py-8 uppercase text-[10px] md:text-[11px] tracking-[0.5em] font-black rounded-sm transition-all duration-700 shadow-2xl
            ...
            {step === 3 && (
              <div className="space-y-8 md:space-y-12 animate-fade-up text-charcoal pb-12">
                <div className="text-center space-y-2 md:space-y-4">
                  <h3 className="font-serif text-3xl md:text-6xl">O Seu Resumo</h3>
                  <p className="text-charcoal/40 text-xs md:text-sm italic font-light">Revise os detalhes antes de concluir a sua experiência real.</p>
                </div>
                ...
                {isPrestigeGuest && (
                  <div className="bg-charcoal text-white p-6 md:p-8 border-l-4 border-gold rounded-sm shadow-xl flex justify-between items-center group overflow-hidden relative">
                    ...
                    <div className="relative z-10">
                      <span className="text-gold text-[9px] md:text-[10px] uppercase tracking-[0.5em] font-black block mb-2">Estatuto de Prestígio</span>
                      <h4 className="font-serif text-2xl md:text-3xl">Hóspede Privilegiado</h4>
                      <p className="text-white/50 text-[9px] md:text-[10px] uppercase tracking-[0.2em] mt-2 font-bold italic">Amenidades de Boas-Vindas & Upgrade de Experiência Incluídos.</p>
                    </div>
                ...
                <div className="bg-white border border-charcoal/5 p-6 md:p-14 rounded-sm shadow-2xl space-y-10 md:space-y-16 relative">
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12 border-b border-charcoal/5 pb-10 relative z-10">
                      ...
                      <div>
                        <span className="text-[9px] md:text-[10px] uppercase tracking-[0.4em] text-gold font-black block mb-2">Noites</span>
                        <span className="text-sm md:text-lg font-serif [font-variant-numeric:lining-nums]">{nights} Noite{nights !== 1 ? 's' : ''}</span>
                      </div>
                      <div>
                        <span className="text-[9px] md:text-[10px] uppercase tracking-[0.4em] text-gold font-black block mb-2">Comitiva</span>
                        <span className="text-sm md:text-lg font-serif [font-variant-numeric:lining-nums]">{adultCount} Ad. {childrenAges.length > 0 && `/ ${childrenAges.length} Cri.`}</span>
                      </div>
                   </div>
                   ...
                   <div className="space-y-8 relative z-10">
                      <div className="flex items-center gap-3 border-l-2 border-gold pl-4">
                        ...
                        <h4 className="text-[10px] md:text-[11px] uppercase tracking-[0.5em] font-black text-charcoal/40">Detalhamento de Custos</h4>
                      </div>
                          ...
                          <div key={room.id} className="group flex justify-between items-start">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-charcoal/40 uppercase tracking-widest font-bold">{nights} Noites × €{room.price}</span>
                              </div>
                            </div>
                          ...
                          return (
                            <div key={`child-${i}`} className="flex justify-between items-start opacity-80 pt-2">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <span className="text-[9px] md:text-[11px] uppercase tracking-[0.2em] font-black text-charcoal/60 block">Suplemento Criança {i+1} ({age} anos)</span>
                              ...
                              <span className="text-xs md:text-sm font-black text-gold [font-variant-numeric:lining-nums]">
                                {itemTotal > 0 ? `€${itemTotal}` : 'Cortesia'}
                              </span>
                            </div>
                          );
                        })}
                   ...
                   <div className="pt-8 md:pt-12 flex flex-col md:flex-row justify-between items-center md:items-end border-t border-charcoal/10 relative z-10 gap-8">
                      <div className="text-center md:text-left space-y-2">
                        <span className="block text-[10px] md:text-[12px] uppercase tracking-[0.5em] font-black text-charcoal/20">Investimento na sua Tranquilidade</span>
                        <div className="flex flex-wrap justify-center md:justify-start gap-4">
                           <div className="flex items-center gap-2 text-[9px] uppercase tracking-widest font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full">
                              <Check className="w-3 h-3" /> Cancelamento Flexível
                           </div>
                           <div className="flex items-center gap-2 text-[9px] uppercase tracking-widest font-bold text-gold bg-gold/5 px-3 py-1 rounded-full">
                              <Check className="w-3 h-3" /> Pequeno-almoço Incluído
                           </div>
                        </div>
                      </div>
                      <div className="text-center md:text-right shrink-0">
                        <span className="block text-[9px] md:text-[11px] uppercase tracking-[0.6em] font-black text-gold/40 mb-1">Total Final da Estadia</span>
                        ...
                        <p className="text-[8px] text-charcoal/30 uppercase tracking-[0.4em] mt-2 font-black">IVA e Taxas Turísticas Incluídas</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="pt-4">
                  <button 
                    type="button" 
                    onClick={handleNext} 
                    className="w-full bg-charcoal text-white py-6 md:py-9 uppercase text-[11px] md:text-[13px] tracking-[0.6em] font-black hover:bg-gold transition-all duration-700 shadow-2xl rounded-sm flex items-center justify-center gap-4 group"
                  >
            ...
            {step === 4 && (
              <div className="space-y-8 md:space-y-12 animate-fade-up text-charcoal pb-12">
                <div className="text-center space-y-2 md:space-y-4">
                  <h3 className="font-serif text-3xl md:text-5xl">Confirmação & Faturação</h3>
                  <div className="flex items-center justify-center gap-2 text-gold">
                    <Lock className="w-3 h-3 md:w-4 md:h-4" />
                    <span className="text-[10px] uppercase tracking-widest font-bold">Portal de Reserva Seguro</span>
                  </div>
                </div>

                <div className="bg-white p-6 md:p-8 rounded-sm shadow-lg border border-charcoal/5">
                  <h4 className="text-[10px] md:text-[11px] uppercase tracking-[0.4em] font-black text-gold mb-6 md:mb-8">Dados de Contacto</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-charcoal/60">Nome Próprio</label>
                      <input 
                        type="text" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-charcoal/60">Apelido</label>
                      <input 
                        type="text" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-charcoal/60">Telefone</label>
                      <input 
                        type="tel" 
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-charcoal/60">NIF (Opcional)</label>
                      <input 
                        type="text" 
                ...
                <div className="bg-charcoal text-white p-8 md:p-12 rounded-sm shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 text-center space-y-6">
                    ...
                    <h4 className="text-xs uppercase tracking-[0.4em] font-black text-gold">Pagamento no Hotel & Confirmação via E-mail</h4>
                    <p className="font-serif text-lg md:text-xl leading-relaxed italic text-white/80">
                      "A sua reserva será validada pela nossa equipa de curadoria. Todos os detalhes para a garantia da reserva e métodos de pagamento ser-lhe-ão enviados diretamente por e-mail."
                    </p>
                    <div className="pt-4 flex flex-col items-center gap-3">
                       <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-white/40">
                          <Check className="w-3 h-3 text-gold" /> Pagamento final realizado no Check-in
                       </div>
                       <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-white/40">
                          <Check className="w-3 h-3 text-gold" /> Suporte personalizado via correio eletrónico
                       </div>
                    </div>
                  </div>
                </div>
                ...
                <button 
                  type="button" 
                  onClick={handleNext} 
                  className="w-full bg-gold text-white py-6 md:py-8 uppercase text-[10px] md:text-[12px] tracking-[0.6em] font-black hover:bg-charcoal transition-all duration-700 shadow-2xl rounded-sm flex items-center justify-center gap-3"
                >
                  <CalendarIcon className="w-4 h-4" />
                  Confirmar Pré-Reserva de €{totalPrice}
                </button>
                <p className="text-center text-[10px] text-charcoal/30 uppercase tracking-[0.4em] font-black">Ao clicar, confirma a receção de detalhes via email.</p>
              </div>
            )}
            ...
            {step === 5 && (
              <div className="text-center py-16 md:py-24 space-y-8 md:space-y-12 animate-fade-up">
                ...
                <div className="space-y-4 md:space-y-6">
                  <h3 className="font-serif text-4xl md:text-5xl text-charcoal">Solicitação Enviada</h3>
                  <div className="bg-gold/5 p-6 rounded-sm border border-gold/10 max-w-sm mx-auto">
                    <p className="text-charcoal/60 leading-relaxed font-light text-sm md:text-base italic">
                      Verifique a sua caixa de entrada. Receberá os detalhes para o pagamento e confirmação oficial em breve.
                    </p>
                  </div>
                </div>
                <div className="pt-6 md:pt-10"><button type="button" onClick={onClose} className="bg-gold text-white px-10 py-4 md:px-16 md:py-6 uppercase text-[9px] md:text-[11px] tracking-[0.5em] font-black hover:bg-charcoal rounded-sm transition-all">Voltar ao Palácio</button></div>
              </div>
            )}
          </div>
        </div>
